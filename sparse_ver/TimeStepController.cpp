...
void TimeStepController::step_liu(SimulationModel& model, Real h)
{
	START_TIMING("simulation step");
	TimeManager* tm = TimeManager::getCurrent();

	Vector3r grav = getGrav();
...
void TimeStepController::step(SimulationModel &model)
{
	...
	for (unsigned int step = 0; step < m_subSteps; step++)
	{
		...
		#pragma omp parallel if(numBodies > MIN_PARALLEL_SIZE) default(shared)    
		{
			...
			for (int i = 0; i < numBodies; i++)
			{
				if (i == 2 && rb[i]->getVelocity().z() != 0) {
					printf("%.15lf %.15lf %.15lf", rb[i]->getPosition().z(), rb[i]->getOldPosition().z(), rb[i]->getVelocity().z());
					printf("fuck2\n");
					while (1);
				}
				...
				{
					/*
					Real k = sqrt(2*(rb[i]->getMomentum() - grav.norm()*rb[i]->getPosition().y()));
					Vector3r v = rb[i]->getVelocity();
					if (i == 2 && rb[i]->getVelocity().z() != 0) {
						printf("%.25lf, %.25lf\n", rb[i]->getPosition().z(), rb[i]->getOldPosition().z());
						printf("%.25lf %.25lf %.25lf", rb[i]->getPosition().z(), rb[i]->getOldPosition().z(), rb[i]->getVelocity().z());
						printf("fuck1\n");
						while (1);
					}
					v.normalize();

					rb[i]->setVelocity(v * k);
					*/
				}
			}
