...
void Liu13_ClothModel::setParticleMass(ParticleData& pd)
{
	...
	for (unsigned int i = 0; i < m_pindices.size(); i++)
	{
		pd.setMass(m_pindices[i], 0.05);
	}
...
void Liu13_ClothModel::setJMatrix()
{
	...
	m_J.resize(m_pindices.size() * 3, edge.size() * 3);
...
void Liu13_ClothModel::getForceVector(ParticleData& pd, Real h)
{
	...
	for (long long i = 0; i < m; i++)
	{
		...
		pd.setVelocity(m_pindices[i], Vector3r(0,0,0));
...
void Liu13_ClothModel::setVectorSize()
{
	...
	m_XOld.resize(3 * m_pindices.size());
...
void Liu13_ClothModel::vectorToPosition(ParticleData& pd)
{
	for (long long i = 0; i < m_pindices.size(); i++)
	{
		pd.getLastPosition(m_pindices[i]) = pd.getOldPosition(m_pindices[i]);
		pd.getOldPosition(m_pindices[i]) = pd.getPosition(m_pindices[i]);
		pd.getPosition(m_pindices[i]) = Vector3r(m_X(3 * i), m_X(3 * i + 1), m_X(3 * i + 2));
	}
}
	...
	for (unsigned int i = 0; i < edge.size(); i++)
	{
		...
		if (diff > 0.1 * r)
		{
			...
			*/
			...
			{
				pd.getVelocity(m_pindices[v1]) -= p12 / h2;
				//m_X.segment(3 * v1, 3) = pd.getPosition(m_pindices[v1]);
				//save.push_back(v1);
				pd.getVelocity(m_pindices[v2]) += p12 / h2;
				//m_X.segment(3 * v2, 3) = pd.getPosition(m_pindices[v2]);
				//save.push_back(v2);
			}
			else if (v2fix)
			{
				pd.getVelocity(m_pindices[v1]) -= 2 * p12 / h2;
				//m_X.segment(3 * v1, 3) = pd.getPosition(m_pindices[v1]);
				//save.push_back(v1);
			}
			else 
			{
				pd.getVelocity(m_pindices[v2]) += 2 * p12 / h2;
				//m_X.segment(3 * v2, 3) = pd.getPosition(m_pindices[v2]);
				//.push_back(v2);
			}
...
void Liu13_ClothModel::collisionSphere(ParticleData& pd)
{
	...
	for (unsigned int i = 0; i < m_pindices.size(); i++)
	{
		...
		pd.getPosition(m_pindices[i]) = center + dir * (radius+0.01);
		//m_XOld.segment(3 * i, 3) = pd.getPosition(m_pindices[i]) - dir * length;
		//m_XOld.segment(3 * i, 3) = m_X.segment(3 * i, 3);
		m_X.segment(3 * i, 3) = pd.getPosition(m_pindices[i]);
	}
}
